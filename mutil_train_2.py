...
def main():
    ...
    # 初始化模型
    Hnet = stegTransX(in_channels=9, out_channels=3)
    Rnet = stegTransX(in_channels=3, out_channels=6)
    ...
    for epoch in range(c.epochs):
        loop = tqdm((zip(dataset.trainloader, dataset.trainloader1, dataset.trainloader2)),total = len(dataset.trainloader),leave=True)
        Hnet.train()
        Rnet.train()
        total_loss = 0.0

        for data1, data2, data3 in loop:
            ...
            all_secret = torch.cat((secret1, secret2), 1)
        ...
        # 验证循环
        loop = tqdm((zip(dataset.testloader, dataset.testloader1, dataset.testloader2)), total=len(dataset.testloader), leave=True)
        Hnet.eval()
        Rnet.eval()
        ...
        with torch.no_grad():
            ...
            for data1, data2, data3 in loop:
                ...
                Rsecret2=R_output[:, 3:, :, :].clamp(0, 1).cpu()
            ...
            logging.info(
                f'Val Epoch [{epoch}/{c.epochs}] val_loss: {total_loss / len(dataset.testloader)} psnr_c: {np.mean(psnr_c)} psnr_s1: {np.mean(psnr_s1)} psnr_s2: {np.mean(psnr_s2)} y_psnr_c:{np.mean(y_psnr_c)} y_psnr_s1:{np.mean(y_psnr_s1)} y_psnr_s2:{np.mean(y_psnr_s2)}')
